...
contract FX1SportsToken is Ownable, IFX1SportsToken {
    ...
    /// @notice Maps addresses to indicate whether they are considered bots
    mapping(address => bool) public bots;
    /// @notice Maps addresses to indicate whether they are excluded from transaction fees
    mapping(address => bool) public excludedFromFees;
    /// @notice Maps addresses to indicate whether they are excluded from maximum transfer limits
    ...
    /// @notice Maximum amount of tokens that can be transferred in a single transaction
     ...
     * @param _newPair The new Uniswap pair address
     */
    function updatePair(address _newPair) external onlyOwner {
        require(_newPair != address(0), "Invalid pair address");
        excludedFromMaxTransfer[pair] = false;
        excludedFromMaxWallet[pair] = false;
        whitelists[pair] = false;
        pair = _newPair;
        excludedFromMaxTransfer[_newPair] = true;
        excludedFromMaxWallet[_newPair] = true;
        whitelists[_newPair] = true;
    }
    ...
    /**
     * @notice External function allows the contract owner to mark multiple addresses as bots, preventing them from executing certain functions
     * @param _bots An array of addresses to be marked as bots
     */
    function setBots(address[] memory _bots) external override onlyOwner {
        ...
        for (uint256 i = 0; i < _bots.length; ) {
            bots[_bots[i]] = true;
    ...
    /**
     * @notice External function allows the contract owner to remove an address from the list of marked bot addresses
     * @param _notbot The address to be removed from the list of bot addresses
     */
    function delBot(address _notbot) external override onlyOwner {
        bots[_notbot] = false;
    }
    ...
    /**
     * @notice External function allows the contract owner to exclude or include multiple addresses from the list of addresses exempted from maximum transfer limits
    ...
    /**
     * @notice External function allows the contract owner to set a new maximum transfer amount limit
    ...
    function _taxonSellTransfer(
        ...
        (uint16 marketingFeeRate, uint16 liquidityFeeRate) = _getSellFeeRate();

        uint256 feeAmount = (_amount * marketingFeeRate) / FIXED_POINT;
        uint256 liquidityFeeAmount = (_amount * liquidityFeeRate) / FIXED_POINT;
        uint256 marketingFeeAmount = feeAmount - liquidityFeeAmount;
    ...
    function _taxonBuyTransfer(
        ...
        (uint16 marketingFeeRate, uint16 liquidityFeeRate) = _getBuyFeeRate();
    ...
    function _getSellFeeRate()
        internal
        view
        returns (uint16 _marketingFeeRate, uint16 _liquidityFeeRate)
    {
